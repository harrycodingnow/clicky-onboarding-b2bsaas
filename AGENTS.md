...
**Floating Dock Pattern**: Alongside the menu bar icon, an always-visible floating dock (`FloatingDockManager`) hosts the same `CompanionPanelView` in a draggable, non-activating `NSPanel`. It defaults to the top-center of the main screen, remembers its last position across launches (stored as its top-left corner in `UserDefaults`), and keeps its top edge anchored as the panel's content height changes. Because the dock is always on screen, the menu bar drop-down is no longer auto-opened on launch. Like the cursor overlay, it joins all Spaces and is excluded from screenshots since `CompanionScreenCaptureUtility` filters out all app-owned windows.
...
| File | Lines | Purpose |
|------|-------|---------|
| `leanring_buddyApp.swift` | ~95 | Menu bar app entry point. Uses `@NSApplicationDelegateAdaptor` with `CompanionAppDelegate` which creates `MenuBarPanelManager`, shows the always-visible `FloatingDockManager`, and starts `CompanionManager`. No main window — the app lives in the status bar plus a floating dock. |
...
| `FloatingDockManager.swift` | ~215 | Always-visible, draggable floating dock that hosts the full `CompanionPanelView` (with its close button hidden) in a non-activating `NSPanel`. Defaults to top-center of the main screen, persists its top-left position across launches in `UserDefaults`, top-anchors on content resize, and clamps to the current screen. |
| `CompanionPanelView.swift` | ~767 | SwiftUI panel content shown in both the menu bar dropdown and the floating dock. Shows companion status, push-to-talk instructions, model picker (Sonnet/Opus), permissions UI, DM feedback button, and quit button. `showsDismissButton` hides the close (X) button when hosted in the always-visible dock. Dark aesthetic using `DS` design system. |
