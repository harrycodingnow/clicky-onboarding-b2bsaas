...
struct CompanionPanelView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 0) {
      ...
      // Employee onboarding checklist — the core of the B2B flow. Shown
      // once the new hire has granted the permissions Clicky needs.
      if companionManager.allPermissionsGranted {
        Spacer()
          .frame(height: 16)

        OnboardingPanelView(companionManager: companionManager)
          .padding(.horizontal, 16)
      }
  ...
  private var permissionsCopySection: some View {
    if companionManager.hasCompletedOnboarding && companionManager.allPermissionsGranted {
      ...
      VStack(alignment: .leading, spacing: 6) {
        ...
        Text(
          "Nothing runs in the background. Clicky will only take a screenshot when you press the hot key. So, you can give that permission in peace. If you are still sus, eh, I can't do much there champ."
        )
        .font(.system(size: 11))
